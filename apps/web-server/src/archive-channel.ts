...
const FORMATS: ReadonlyArray<ArchiveFormat> = ["zip", "tar", "tar.gz"];
  ...
  (request) =>
    Effect.gen(function* () {
      ...
      if (repoIdRaw === null || treeish === null || format === null) {
        return Http.HttpServerResponse.text("missing repoId/treeish/format", {
          status: 400,
        });
      }
      if (!FORMATS.includes(format as ArchiveFormat)) {
        return Http.HttpServerResponse.text("invalid format", { status: 400 });
      }
