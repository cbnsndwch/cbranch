...
export default defineConfig({
    ...
    server: {
        ...
        // Vite runs the proxy on behalf of browsers that may reach it over a LAN IP.
        // Rewrite the forwarded Origin so the loopback-only backend continues to see
        // only its trusted local origin rather than exposing its allowlist to the LAN.
        proxy: {
            '/rpc': {
                ...
                configure: proxy => {
                    proxy.on('proxyReq', request =>
                        request.setHeader('origin', 'http://localhost:7420'),
                    );
