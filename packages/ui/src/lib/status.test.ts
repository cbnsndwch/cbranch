...
describe('isUnstagedChange', () => {
    test('returns true for worktree modified', () => {
        expect(isUnstagedChange(makeEntry({ unstaged: 'modified' }))).toBe(
            true,
        );
    });
...
describe('statusLabel', () => {
    test("modified returns 'modified'", () => {
        expect(statusLabel(makeEntry({ unstaged: 'modified' }))).toBe(
            'modified',
        );
    });
    ...
    test('renamed without origPath falls back to path', () => {
        expect(
            statusLabel(makeEntry({ staged: 'renamed', path: 'new.ts' })),
        ).toBe('renamed → new.ts');
    });
    ...
    test("untracked returns 'untracked'", () => {
        expect(
            statusLabel(
                makeEntry({ unstaged: 'untracked', isUntracked: true }),
            ),
        ).toBe('untracked');
    });
