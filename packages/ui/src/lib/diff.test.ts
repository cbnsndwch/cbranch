...
describe("buildDiffSpec (P1-DET-3 / P1-DIFF-5)", () => {
  test("maps options to the wire DiffSpec; combined drops the base", () => {
    const spec = buildDiffSpec(repoId, "abc", {
    ...
    const combined = buildDiffSpec(repoId, "abc", {
      ...defaultDiffOptions,
      base: "p2",
      combined: true,
    });
...
describe("classification helpers", () => {
  test("submodule detected by the 160000 mode (P1-DIFF-10)", () => {
    expect(
      isSubmodule(mkFile({ newPath: "vendor/lib", newMode: "160000" })),
    ).toBe(true);
...
describe("file tree (P1-DIFF-2)", () => {
  ...
  test("flatRows is one row per file, sorted by path", () => {
    expect(flatRows(files).map((r) => r.path)).toEqual([
      "readme.md",
      "src/a.ts",
      "src/sub/b.ts",
    ]);
  ...
  test("treeRows hides collapsed subtrees", () => {
    ...
    expect(
      treeRows(tree, expandedAll).some((r) => r.path === "src/sub/b.ts"),
    ).toBe(true);
