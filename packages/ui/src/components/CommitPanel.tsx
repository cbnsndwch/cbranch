...
export const CommitPanel = forwardRef<CommitPanelHandle, CommitPanelProps>(
  function CommitPanel(
    ...
    if (hasConflicts) disabledReason = "Resolve conflicts before committing";
    else if (authorIncomplete) disabledReason = "Enter author name and email";
    ...
    const handleAmendToggle = (next: boolean) => {
      ...
      if (next && subjectEmpty && commitDraft.body.trim() === "") {
        ...
        if (msg) updateCommitDraft({ subject: msg.subject, body: msg.body });
      }
    };
    ...
    const handleCommit = () => {
      ...
      const input: CommitInput = {
        ...
        amend: amendActive,
        resetAuthor: amendActive && commitDraft.resetAuthor ? true : undefined,
        signoff: commitDraft.signoff,
        sign: commitDraft.sign ? { format: commitDraft.signFormat } : undefined,
            ...
            <div className="flex flex-col gap-0.5 px-2 pb-1">
              {isUnborn && (
                <p className="text-muted-foreground text-[11px]">
                  Unborn branch — this will create the first commit. Amend,
                  reset author and reuse-message are unavailable.
                </p>
              )}
              {isDetached && (
                <p className="text-muted-foreground text-[11px]">
                  Detached HEAD — committing here won&apos;t advance any branch.
                </p>
              )}
              {amendOfPushed && (
                <p className="text-amber-600 text-[11px] dark:text-amber-400">
                  ⚠ Amending a commit that appears already pushed — you&apos;ll
                  need a force-push afterwards.
                </p>
              )}
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 px-2 py-1">
            ...
            {amendActive && (
              <span className="flex items-center gap-1.5">
                <Switch
                  ...
                  onCheckedChange={(v) => updateCommitDraft({ resetAuthor: v })}
            ...
            <span className="flex items-center gap-1.5">
              <Switch
                ...
                onCheckedChange={(v) => updateCommitDraft({ sign: v })}
                aria-label="Sign"
              />
              <span>Sign</span>
            </span>
            {commitDraft.sign && (
              <select
                ...
                onChange={(e) =>
                  updateCommitDraft({
                    signFormat: e.target.value as "gpg" | "ssh",
                  })
                }
            ...
            <span className="flex items-center gap-1.5">
              <Switch
                ...
                onCheckedChange={(v) => updateCommitDraft({ allowEmpty: v })}
          ...
          {commitDraft.authorOverride && (
            <div className="flex items-center gap-2 border-t px-2 py-1">
              <input
                ...
                onChange={(e) =>
                  updateCommitDraft({ authorName: e.target.value })
                }
              ...
              <input
                ...
                onChange={(e) =>
                  updateCommitDraft({ authorEmail: e.target.value })
                }
                ...
                <Tooltip>
                  ...
                  >
                    <Button
                      ...
                      disabled={
                        disabledReason !== null || commitCreate.isPending
                      }
                      className="h-7 text-xs"
                    >
