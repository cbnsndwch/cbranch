...
function CleanDialogBody({ repoId }: { repoId: RepoId }) {
  ...
  const runClean = () =>
    clean.mutate(
      ...
      {
        onSuccess: (result) => {
          toast.success(`Removed ${result.removed} ${plural(result.removed)}`);
          reset();
          setOpen(false);
        },
      ...
      >
        <DialogContent style={{ width: "min(560px, 92vw)" }}>
          <div className="flex flex-col gap-3 p-4">
            ...
            <DialogDescription>
              Permanently remove untracked files. Preview first — only the
              previewed entries are removed. Tracked and staged files are never
              touched.
            </DialogDescription>
            ...
            <label className="flex items-center gap-2 text-sm">
              <Checkbox
                ...
                checked={directories}
                onCheckedChange={(c) => setDirectories(c === true)}
                disabled={pending}
              />
            ...
            <div className="flex items-center gap-2">
              <Button
                size="sm"
                variant="outline"
                onClick={() => setPreviewed({ directories, ignored })}
