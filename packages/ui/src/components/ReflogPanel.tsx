...
export function ReflogPanel({
    ...
    const refOptions = useMemo(
        () => [
            'HEAD',
            ...(branches.data?.localBranches.map(b => b.name) ?? []),
        ],
        [branches.data],
    );
    ...
    const reset = (oid: Oid, mode: 'soft' | 'mixed' | 'hard') =>
        resetTo.mutate(
            ...
            {
                onSuccess: () =>
                    toast.success(`Reset (${mode}) to ${shortOid(oid)}`),
    ...
    return (
        <div className="flex h-full flex-col">
            <div className="flex items-center gap-2 border-b px-3 py-2">
                ...
                <Select
                    value={ref}
                    onValueChange={value => setRef(value ?? 'HEAD')}
                >
                    <SelectTrigger
                        aria-labelledby="reflog-ref-label"
                        className="w-48"
                    >
                        ...
                        {entries.map(e => (
                            ...
                            >
                                ...
                                <DropdownMenu>
                                    ...
                                    >
                                        <DropdownMenuItem
                                            onClick={() => setBranchFrom(e.oid)}
                                        >
                                            Create branch here…
                                        </DropdownMenuItem>
                                        <DropdownMenuItem
                                            onClick={() => reset(e.oid, 'soft')}
                                        >
                                            Reset (soft) to here
                                        </DropdownMenuItem>
                                        <DropdownMenuItem
