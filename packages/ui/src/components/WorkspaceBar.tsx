        ...
        {dirty ? (
          <span
            className={cn(
              ...
              conflicted ? "bg-destructive" : "bg-status-unstaged",
            )}
            aria-label={conflicted ? "Conflicts" : "Uncommitted changes"}
          />
        ) : null}
      </button>
      <button
...
export function WorkspaceBar() {
  ...
  useEffect(() => {
    if (
      !engagement ||
      !activeRepoId ||
      !engagement.repositories.some((repo) => repo.repoId === activeRepoId) ||
      session.isPending
    )
      return;
      ...
      <div className="bg-muted/60 flex h-8 min-w-0 shrink-0 items-stretch border-b">
        <button
          ...
          className={cn(
            ...
            activeRepoId === null ? "bg-background" : "hover:bg-accent/60",
          )}
        >
          <WorkspaceAvatar
