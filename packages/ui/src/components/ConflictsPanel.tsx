...
export function ConflictsPanel({ repoId, onEdit }: ConflictsPanelProps) {
        ...
        continueMut.mutate(msg ? { message: msg } : {}, {
            onSuccess: r => {
                ...
                if (r.outcome === 'completed')
                    ...
                    toast.message(
                        'Nothing to commit — skip or commit an empty change',
                    );
            },
            onError: err => toast.error(String(err)),
        });
    ...
    const busy =
        continueMut.isPending || abortMut.isPending || skipMut.isPending;
        ...
        <div className="bg-muted border-b px-3 py-2" role="status">
            <div className="flex items-center justify-between gap-2">
                <div className="text-xs font-medium">
                    ...
                    {progress && (
                        <span className="ml-1">
                            — {isRebase ? 'step' : 'commit'} {progress.current}{' '}
                            of {progress.total}
                        </span>
                    )}
                ...
                {(file.isBinary || file.isSubmodule) && (
                    <p className="text-muted-foreground mt-0.5 text-[10px]">
                        {file.isSubmodule
                            ? 'Submodule reference'
                            : 'Binary file'}{' '}
                        — can&apos;t be merged line-by-line; resolve by taking
                        one whole side.
                    </p>
                )}
            </div>
