...
describe('BlamePanel (REQ-UX-009)', () => {
    ...
    test('a block popover reveals full details and opens the commit (REQ-BL-005)', async () => {
        ...
        act(() =>
            fireEvent.click(
                screen.getByLabelText('Blame details for aaaaaaaa'),
            ),
        );
    ...
    test('Blame previous re-blames the parent at the prior path (REQ-BL-004/AC-12)', async () => {
        ...
        act(() =>
            fireEvent.click(screen.getByLabelText('Back to previous blame')),
        );
        await waitFor(() =>
            expect(
                screen.queryByLabelText('Back to previous blame'),
            ).toBeNull(),
        );
    });
    ...
    test('shows a loading skeleton while blame is pending (REQ-BL-006/UX-011)', async () => {
        ...
        renderPanel(
            makeApi({ blame: blame as unknown as CbranchApi['blame'] }),
        );
        await waitFor(() =>
            expect(
    ...
    test('Blame previous is disabled at a root with no previous (boundary)', async () => {
        ...
        act(() =>
            fireEvent.click(
                screen.getByLabelText('Blame details for bbbbbbbb'),
            ),
        );
