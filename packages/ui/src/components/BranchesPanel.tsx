...
export function BranchesPanel({ repoId }: BranchesPanelProps) {
    ...
    const handleDetach = (ref: string) => {
        detachMut.mutate(
            ...
            {
                onSuccess: () =>
                    toast.success('Checked out ' + ref + ' (detached)'),
                onError: err => toast.error(String(err)),
            },
        );
    };
        ...
        switchMut.mutate(
            { target },
            {
                onError: err => {
                    ...
                    if (
                        (err as { code?: unknown }).code === 'dirtyWorkingTree'
                    ) {
        ...
        switchMut.mutate(
            {
                ...
                strategy,
                stashAndReapply:
                    strategy === 'stash' ? stashReapply : undefined,
            },
            {
                onSuccess: () => {
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            You have local changes. How do you want to handle
                            them when switching branches?
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <div className="py-2">
                        <label className="flex items-center gap-2 text-sm">
