...
export function BlamePanel({
    ...
    const commits = useMemo(
        () =>
            blame
                ? indexCommits(blame.commits)
                : new Map<string, BlameCommit>(),
        [blame],
    );
                ...
                {tooLarge && (
                    <div className="flex flex-1 flex-col items-start gap-2 p-6 text-sm">
                        <p className="font-medium">
                            File too large to blame in app
                        </p>
                        <p className="text-muted-foreground">
                            {tooLarge.byteSize.toLocaleString()} bytes exceeds
                            the inline blame limit. Blaming it anyway may be
                            slow.
                        </p>
                        <Button
                            size="sm"
...
function BlameSkeleton() {
    return (
        <div
            className="flex-1 space-y-1 overflow-hidden p-3"
            aria-hidden="true"
        >
