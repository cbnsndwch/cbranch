...
const STATUS_STYLE: Record<SubmoduleStatus, { label: string; cls: string }> = {
    ...
    outOfSync: {
        label: 'out of sync',
        cls: 'border-orange-400 text-orange-600',
    },
...
function SubmoduleStatusBadge({ status }: { status: SubmoduleStatus }) {
    ...
    return (
        <span className={`rounded border px-1 text-[9px] ${s.cls}`}>
            {s.label}
        </span>
    );
}
            ...
            >
                <AlertDialogContent>
                    ...
                    <div className="py-2 text-sm">
                        Force-updating{' '}
                        <span className="font-mono">{forceTarget?.path}</span>{' '}
                        overwrites any local changes in the submodule's working
                        tree.
                    </div>
                    <AlertDialogFooter>
