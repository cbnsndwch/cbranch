...
const makeApi = (overrides: Partial<CbranchApi> = {}): CbranchApi =>
  ({
    ...
    gc: vi.fn(
      async () =>
        new GcResult({ stdout: "Counting objects: 5, done.\n", stderr: "" }),
    ),
...
describe("GcDialog", () => {
  ...
  test("Run sends the selected options, shows captured output, toasts, invalidates", async () => {
    ...
    expect(await screen.findByText(/Counting objects: 5, done\./)).toBeTruthy();
    expect(toast.success).toHaveBeenCalled();
    // REQ-P5-GC-004: explicit refs + commits invalidation.
    await waitFor(() => {
      expect(invalidate).toHaveBeenCalledWith({ queryKey: [repoId, "refs"] });
