...
const looksLikePath = (value: string): boolean => value.startsWith("/") || (value.length > 2 && value[1] === ":");
...
export function CommandPalette() {
  ...
  const matches = (recent.data ?? []).filter(
    (r) => term === "" || r.name.toLowerCase().includes(term) || r.path.toLowerCase().includes(term),
  );
        ...
        <Command shouldFilter={false} label="Open or switch repository">
          ...
          <Command.List className="max-h-80 overflow-auto p-1">
            ...
            {looksLikePath(query) ? (
              ...
              >
                Open path: <span className="font-mono text-xs">{query.trim()}</span>
              </Command.Item>
            ) : null}
            {matches.length === 0 && !looksLikePath(query) ? (
              <div className="text-muted-foreground px-3 py-2 text-xs">No recent repositories.</div>
            ) : null}
            {matches.map((repo) => (
              <Command.Item
                key={repo.repoId}
              ...
              >
                ...
                <span className="text-muted-foreground truncate text-xs">{repo.path}</span>
              </Command.Item>
            ))}
          </Command.List>
        </Command>
