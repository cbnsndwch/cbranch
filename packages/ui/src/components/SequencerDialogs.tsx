...
function LoadFailedNote() {
    return (
        <p className="text-destructive text-[11px]" role="alert">
            Could not load this commit&apos;s details. Reopen the dialog to try
            again.
        </p>
    );
}
    ...
    return (
        ...
        >
            <p className="text-muted-foreground">
                Apply {commits.length > 1 ? 'these commits' : 'this commit'}{' '}
...
function EmptyPickDialog({
    ...
    return (
        ...
        >
            <p className="text-muted-foreground">
                This {opLabel.toLowerCase()} produced no changes — the commit is
                already applied or empty.
            </p>
            {currentOid && (
            ...
            <p className="text-muted-foreground text-[11px]">
                Skip it to move on, or record an empty commit to keep it in the
                history.
            </p>
        </DialogShell>
    );
