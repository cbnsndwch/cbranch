...
export function MergeEditor({ repoId, path, onClose }: MergeEditorProps) {
  ...
  useEffect(() => {
    ...
    setCompareSide(
      data.theirs.present ? "theirs" : data.ours.present ? "ours" : "base",
    );
    setCmEpoch((e) => e + 1);
  }, [data]);
  ...
  const parsed = useMemo(
    () =>
      resultText === null
        ? { blocks: [] as ReadonlyArray<ConflictBlock>, ambiguous: false }
    ...
    void (async () => {
      try {
        ...
        const highlight = async (content: string) => {
          ...
          if (!language) return [];
          const lines = await loadShikiLines({ code: content, language, dark });
          if (!lines) return [];
          ...
          >();
          for (let i = 0; i < lines.length && i < probe.doc.lines; i++) {
        ...
        view = new mergeMod.MergeView({
          a: { doc: compareContent, extensions: [...readOnly, ...aHi] },
  ...
  return (
    ...
    >
      ...
      >
        ...
        {notMergeable && data && (
          <div className="flex-1 p-6 text-sm">
            <p className="font-medium">
              This file can&apos;t be opened in the text merge editor.
            </p>
            ...
            <div className="mt-4 flex gap-2">
              <Button
                ...
                disabled={resolveMut.isPending || !data.ours.present}
              >
                Take ours
              </Button>
              <Button
                onClick={() => wholeFile("theirs")}
                disabled={resolveMut.isPending || !data.theirs.present}
              >
                Take theirs
              </Button>
              <Button
                ...
                disabled={resolveMut.isPending || !data.base.present}
              >
                Take base
              </Button>
            </div>
            ...
            <div className="flex min-h-0 flex-col border-r">
              <div className="bg-muted flex items-center gap-1 border-b px-2 py-1 text-[11px]">
                ...
                {(["base", "ours", "theirs"] as const).map((side) => (
                  ...
                  >
                    {COMPARE_LABELS[side]}
                    {!stageOf(data, side).present ? " (none)" : ""}
                  </button>
                ))}
              </div>
              ...
              {cmFailed && (
                <div className="text-muted-foreground border-t p-2 text-[11px]">
                  Side-by-side compare unavailable; edit the result directly.
                </div>
              )}
            </div>
              ...
              <div className="max-h-[40%] space-y-1.5 overflow-auto border-b p-2">
                {parsed.ambiguous ? (
                  <p className="text-status-behind text-xs">
                    Conflict markers are unbalanced or nested — per-hunk actions
                    are unavailable; edit the result as plain text and save.
                  </p>
                ) : remaining === 0 ? (
                  <p className="text-muted-foreground text-xs">
                    No remaining conflict regions. Review and save.
                  </p>
                ) : (
        ...
        <DialogFooter className="flex-row justify-between border-t px-4 py-2">
          ...
          <div className="flex gap-2">
            ...
            {data?.mergeable && (
              <Button onClick={onSaveClick} disabled={saveMut.isPending}>
                Save &amp; mark resolved
              </Button>
            )}
          </div>
      ...
      <AlertDialog open={markerWarn} onOpenChange={setMarkerWarn}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Conflict markers remain</AlertDialogTitle>
            <AlertDialogDescription>
              The result still contains unresolved conflict markers. Saving now
              writes them to the file, which likely still needs attention.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
