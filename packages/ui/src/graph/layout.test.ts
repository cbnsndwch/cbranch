...
const arrivesAtNode = (row: {
  lane: number;
  segments: ReadonlyArray<{ toLane: number; toY: number }>;
}) => row.segments.filter((s) => s.toY === 0.5 && s.toLane === row.lane);

// Bottom-half edges leaving the node toward a parent lane.
const leavesNode = (row: {
  lane: number;
  segments: ReadonlyArray<{ fromLane: number; fromY: number; toY: number }>;
}) =>
  row.segments.filter(
    (s) => s.fromY === 0.5 && s.toY === 1 && s.fromLane === row.lane,
  );
  ...
  test("linear history is a single straight lane (REQ-GRAPH-edge-cases)", () => {
    ...
    for (const row of rows)
      for (const s of row.segments)
        expect(s.fromLane === 0 && s.toLane === 0).toBe(true);
  });
...
describe("GraphLayout — truncation and stability", () => {
  test("a parent outside the rendered window leaves an open-ended outgoing lane (REQ-GRAPH-007)", () => {
    ...
    const tail = rows[0]!;
    expect(
      tail.segments.some((s) => s.toY === 1 && s.toLane === tail.lane),
    ).toBe(true);
  });
  ...
  test("appending more commits never changes an already-placed row's lane", () => {
    ...
    expect(b.slice(0, head.length).map((r) => r.lane)).toEqual(
      a.map((r) => r.lane),
    );
  });
});
