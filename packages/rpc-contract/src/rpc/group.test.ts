...
const textDiffFile = new DiffFile({
    ...
    hunks: [
        new Hunk({
            ...
            newLines: 2,
            lines: [
                new DiffLine({ kind: 'add', content: 'hello', newLineNo: 2 }),
            ],
        }),
    ],
});
...
describe('CbranchRpcs P1 contract (in-memory RpcTest round-trip)', () => {
    test('every P1 method round-trips schema-valid data', async () => {
        ...
        expect(result.events[0]?.domains).toEqual([
            'status',
            'commits',
            'refs',
        ]);
        // 6.
        expect(result.summaries.map(c => c.subject)).toEqual([
            'first',
            'second',
        ]);
...
describe('CbranchRpcs P4 conflicts/sequencer/blame/file-history round-trip', () => {
    test('every P4 method round-trips schema-valid data', async () => {
        const program = Effect.gen(function* () {
            ...
            const sides = yield* client.ConflictSides({
                repoId,
                path: 'a.txt',
            });
            ...
            const picked = yield* client.CherryPick({
                repoId,
                commits: [oid1],
            });
...
describe('CbranchRpcs P5 power-features round-trip', () => {
    ...
    test('GcPrune rejects an out-of-set literal (RPC-032)', () => {
        expect(
            Exit.isFailure(Schema.decodeUnknownExit(GcPrune)('sometimes')),
        ).toBe(true);
        expect(Exit.isSuccess(Schema.decodeUnknownExit(GcPrune)('now'))).toBe(
            true,
        );
        expect(
            Exit.isSuccess(Schema.decodeUnknownExit(GcPrune)('default')),
        ).toBe(true);
    });
    ...
    test('ArchivePrepare round-trips a descriptor; ArchiveFormat is closed', async () => {
        ...
        expect(
            Exit.isFailure(Schema.decodeUnknownExit(ArchiveFormat)('rar')),
        ).toBe(true);
    });
    ...
    test('SubmoduleStatus is a closed literal set (RPC-032)', () => {
        expect(
            Exit.isSuccess(
                Schema.decodeUnknownExit(SubmoduleStatus)('conflicted'),
            ),
        ).toBe(true);
        expect(
            Exit.isFailure(
                Schema.decodeUnknownExit(SubmoduleStatus)('deleted'),
            ),
        ).toBe(true);
    });
    ...
    test('RebaseStatus decodes every stop reason (incl. execFailed)', () => {
        ...
        expect(
            Exit.isFailure(
                Schema.decodeUnknownExit(RebaseStopReason)('paused'),
            ),
        ).toBe(true);
    });
