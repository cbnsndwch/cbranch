...
export const parseUnbisectable = (text: string): ReadonlyArray<string> => {
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    const line = raw.trim();
    if (HEX40.test(line)) out.push(line);
...
export const computeBisectStatus = (
  ...
  Effect.gen(function* () {
    ...
    const varsResult = yield* runGit({
      cwd,
      args: ["rev-list", "--bisect-vars", "refs/bisect/bad", "--not", ...goods],
      env,
    });
...
export const bisectStart = (
  ...
  Effect.gen(function* () {
    ...
    const seed = bad !== undefined ? [bad, ...(good ?? [])] : [];
    ...
    if (result.exitCode !== 0) {
      return yield* Effect.fail(
        gitError("gitFailed", "git bisect start failed"),
      );
    }
...
export const bisectMark = (
  ...
  Effect.gen(function* () {
    ...
    // good|bad|skip exit 0 even on conclusion; outcome is read from machine state.
    const result = yield* runGit({
      cwd,
      args: ["bisect", mark],
      read: false,
    ...
    return yield* computeBisectStatus(cwd, gitDir, env, markOut);
  });
