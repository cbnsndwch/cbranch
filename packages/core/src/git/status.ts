...
export const parseStatusOutput = (buf: Buffer): WorkingTreeStatus => {
  ...
  while (i < tokens.length) {
    ...
    if (first === "#") {
      ...
      if (key === "branch.oid") {
        branchOid = val === "(initial)" ? undefined : OidBrand.make(val);
        ...
        const a = ab[0];
        const b = ab[1];
        branchAhead = a !== undefined ? parseInt(a.slice(1), 10) : undefined;
        branchBehind = b !== undefined ? parseInt(b.slice(1), 10) : undefined;
      }
      i++;
    } else if (first === "1") {
