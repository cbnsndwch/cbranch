    ...
    Effect.gen(function* () {
        ...
        if (fmt.exitCode !== 0) {
            return yield* Effect.fail(
                gitError(
                    'invalidRefName',
                    `'${safeName}' is not a valid branch name`,
                ),
            );
        }
        ...
        if (existing.localBranches.some(b => b.name === safeName)) {
            return yield* Effect.fail(
                gitError(
                    'refExists',
                    `a branch named '${safeName}' already exists`,
                ),
            );
        }
        ...
        if (startPoint) {
            const safeStart = yield* assertNoLeadingDash(
                startPoint,
                'start point',
            );
            args.push(safeStart);
        ...
        if (strategy === 'stash') {
            ...
            if (reapply !== false) {
                ...
                if (popped.exitCode !== 0) {
                    ...
                    return yield* Effect.fail(
                        gitError('gitFailed', 'stash pop failed after switch', {
                            stderr,
                        }),
                    );
                }
...
export const branchSetUpstream = (
    ...
    Effect.gen(function* () {
        ...
        if (upstream === undefined || upstream === '') {
            ...
            const safeUpstream = yield* assertNoLeadingDash(
                upstream,
                'upstream ref',
            );
            yield* runGitOk({
