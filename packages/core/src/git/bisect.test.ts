...
describe('bisect git operations', () => {
    ...
    test('start checks out a midpoint with populated counts; walk concludes at the first bad', async () => {
        ...
        let status = await run(
            bisectStart(repo.dir, gitDir, oids[5], [oids[0]]),
        );
        ...
        while (status.state === 'bisecting' && guard < 20) {
            // eslint-disable-next-line no-await-in-loop -- bisect is a state machine: each mark depends on the prior checkout.
            status = await run(
    ...
    test('mark with no bisect in progress fails as gitFailed', async () => {
        ...
        const err = await run(
            Effect.flip(bisectMark(repo.dir, gitDir, 'good')),
        );
    ...
    test('goods supplied without a bad fail clearly instead of being dropped', async () => {
        const { repo, gitDir, oids } = await seedRegression(
            'bisect-goods-no-bad',
        );
        const err = await run(
