...
import { type CommitSummary, type DiffFile, type RepoId } from "@cbranch/rpc-contract";
...
import { createFixtureWorkspace, DEFAULT_IDENTITY, type FixtureWorkspace, fixtureDate } from "../testing/fixtures";
...
const logQuery = (over: Partial<ConstructorParameters<typeof LogQuery>[0]> & { repoId: RepoId }): LogQuery =>
  new LogQuery({ limit: 100, ...over });

const diffSpec = (
  over: Partial<ConstructorParameters<typeof DiffSpec>[0]> & { repoId: RepoId; target: string },
): DiffSpec => new DiffSpec({ cached: false, whitespace: "show", context: 3, renames: true, combined: false, ...over });

const byNewPath = (files: ReadonlyArray<DiffFile>, p: string): DiffFile | undefined =>
  files.find((f) => f.newPath === p);
...
describe("log.stream", () => {
  test("an empty (unborn) repo completes with zero rows, not an error", async () => {
    ...
    const rows = await withEngine(cfg, (e) =>
      Effect.gen(function* () {
        ...
        return yield* Stream.runCollect(e.logStream(logQuery({ repoId: h.repoId, limit: 10 })));
      }),
    );
    expect(rows).toEqual([]);
  });
  ...
  test("linear history streams newest-first in topo+date order", async () => {
    ...
    const oids = await withEngine(cfg, (e) =>
      Effect.gen(function* () {
        const h = yield* e.open(repo.dir);
        const rows = yield* Stream.runCollect(e.logStream(logQuery({ repoId: h.repoId })));
  ...
  test("a merge commit exposes its ordered parents", async () => {
    ...
    await repo.commit({ message: "base", files: { "base.txt": "0\n" }, date: fixtureDate(1) });
    await repo.branch("feature");
    const mainWork = await repo.commit({ message: "main", files: { "m.txt": "m\n" }, date: fixtureDate(2) });
    await repo.checkout("feature");
    const featWork = await repo.commit({ message: "feat", files: { "f.txt": "f\n" }, date: fixtureDate(3) });
    await repo.checkout("main");
    await repo.merge("feature", { noFastForward: true, message: "merge feature" });
    ...
    const merge = await withEngine(cfg, (e) =>
      Effect.gen(function* () {
        const h = yield* e.open(repo.dir);
        const rows = yield* Stream.runCollect(e.logStream(logQuery({ repoId: h.repoId, refScope: "all" })));
  ...
  test("a grep filter narrows the result set (AC-8 mapping)", async () => {
    ...
    await repo.commit({ message: "alpha change", files: { "a.txt": "a\n" }, date: fixtureDate(1) });
    await repo.commit({ message: "beta change", files: { "b.txt": "b\n" }, date: fixtureDate(2) });
    await repo.commit({ message: "alpha again", files: { "c.txt": "c\n" }, date: fixtureDate(3) });
    const cfg = newCfg();
    const subjects = await withEngine(cfg, (e) =>
      Effect.gen(function* () {
        const h = yield* e.open(repo.dir);
        const rows = yield* Stream.runCollect(e.logStream(logQuery({ repoId: h.repoId, grep: "alpha" })));
  ...
  test("cursor continuation resumes the same traversal across windows", async () => {
    ...
    const oids = await [1, 2, 3, 4, 5].reduce<Promise<string[]>>(async (accP, i) => {
      const acc = await accP;
      acc.push(await repo.commit({ message: `c${i}`, files: { [`f${i}.txt`]: `${i}\n` }, date: fixtureDate(i) }));
      return acc;
    }, Promise.resolve([]));
    ...
    const all = await withEngine(cfg, (e) =>
      Effect.gen(function* () {
        ...
        const q2 = logQuery({ repoId: h.repoId, limit: 2, cursor: nextLogCursor(q1, page1) ?? undefined });
        const page2 = yield* Stream.runCollect(e.logStream(q2));
        const q3 = logQuery({ repoId: h.repoId, limit: 2, cursor: nextLogCursor(q2, page2) ?? undefined });
        const page3 = yield* Stream.runCollect(e.logStream(q3));
        return [...page1, ...page2, ...page3].map((r: CommitSummary) => r.oid);
      }),
    );
...
describe("commit.detail", () => {
  test("returns structured signatures, split/raw message, and stats", async () => {
    ...
    await repo.commit({ message: "init", files: { "a.txt": "x\n" }, date: fixtureDate(1) });
    ...
    expect(detail.messageRaw).toBe("feat: do thing\n\nThis is the body.\nSecond line.");
  ...
  test("a root commit diffs against the empty tree for its stats", async () => {
    ...
    const cfg = newCfg();
    const detail = await withEngine(cfg, (e) =>
      Effect.flatMap(e.open(repo.dir), (h) => e.commitDetail(h.repoId, OidBrand.make(root))),
    );
...
describe("commit.diff", () => {
  test("classifies add / modify / delete / rename / binary against a base", async () => {
    ...
    const files = await withEngine(cfg, (e) =>
      Effect.flatMap(e.open(repo.dir), (h) => e.commitDiff(diffSpec({ repoId: h.repoId, target: c2, base: c1 }))),
    );
  ...
  test("a root commit diffs against the empty tree (all files added)", async () => {
    ...
    const cfg = newCfg();
    const files = await withEngine(cfg, (e) =>
      Effect.flatMap(e.open(repo.dir), (h) => e.commitDiff(diffSpec({ repoId: h.repoId, target: root }))),
    );
    ...
    const out = await withEngine(cfg, (e) =>
      Effect.gen(function* () {
        ...
        const ctx0 = yield* e.commitDiff(
          diffSpec({ repoId: h.repoId, target: c2, base: c1, context: 0, paths: ["ctx.txt"] }),
        );
        const ctx3 = yield* e.commitDiff(
          diffSpec({ repoId: h.repoId, target: c2, base: c1, context: 3, paths: ["ctx.txt"] }),
        );
        const wsShow = yield* e.commitDiff(diffSpec({ repoId: h.repoId, target: c2, base: c1, paths: ["ws.txt"] }));
        const wsIgnore = yield* e.commitDiff(
          diffSpec({ repoId: h.repoId, target: c2, base: c1, whitespace: "ignore-all", paths: ["ws.txt"] }),
        );
        return {
...
describe("diff.workingFile", () => {
  test("distinguishes staged from unstaged changes", async () => {
    ...
    await repo.commit({ message: "init", files: { "a.txt": "orig\n", "s.txt": "s0\n" }, date: fixtureDate(1) });
    ...
    const out = await withEngine(cfg, (e) =>
      Effect.gen(function* () {
        const h = yield* e.open(repo.dir);
        const unstaged = yield* e.diffWorkingFile(h.repoId, "a.txt", false);
        const staged = yield* e.diffWorkingFile(h.repoId, "s.txt", true);
        const sUnstaged = yield* e.diffWorkingFile(h.repoId, "s.txt", false);
...
describe("file.contentAtRev", () => {
  test("text returns inline utf8; binary returns inline base64", async () => {
    ...
    const out = await withEngine(cfg, (e) =>
      Effect.gen(function* () {
        const h = yield* e.open(repo.dir);
        const text = yield* e.fileContentAtRev(h.repoId, "hello.txt", "HEAD");
        const bin = yield* e.fileContentAtRev(h.repoId, "bin.dat", "HEAD");
        return { text, bin };
      }),
    );
    expect("encoding" in out.text && out.text.encoding).toBe("utf8");
    expect("content" in out.text && out.text.content).toBe("hello\nworld\n");
    expect("isBinary" in out.bin && out.bin.isBinary).toBe(true);
    expect("encoding" in out.bin && out.bin.encoding).toBe("base64");
    expect("content" in out.bin && Buffer.from(out.bin.content, "base64").toString("latin1")).toBe("ab\x00cd");
  });
  ...
  test("content over the 10 MB inline cap returns a side-channel DownloadDescriptor", async () => {
    ...
    await repo.commit({ message: "big", files: { "big.bin": big }, date: fixtureDate(1) });
    const cfg = newCfg();

    const { result, repoId } = await withEngine(cfg, (e) =>
      Effect.gen(function* () {
        const h = yield* e.open(repo.dir);
        const r = yield* e.fileContentAtRev(h.repoId, "big.bin", "HEAD");
        return { result: r, repoId: h.repoId as string };
      }),
    );
    expect("url" in result).toBe(true);
    if ("url" in result) {
      expect(result.url).toBe(`/sidechannel/blob?repoId=${repoId}&rev=HEAD&path=big.bin`);
  ...
  test("an EXTERNAL git commit emits an InvalidationEvent and a subsequent read is not stale", async () => {
    ...
    const out = await withEngine(cfg, (e) =>
      Effect.gen(function* () {
        ...
        const fiber = yield* Effect.forkChild(Stream.runCollect(Stream.take(e.subscribe(repoId), 1)));
        ...
        const external = yield* Effect.promise(() =>
          repo.commit({ message: "external", files: { "b.txt": "b\n" }, date: fixtureDate(2) }),
        );
        const events = yield* Fiber.join(fiber).pipe(Effect.timeout("8 seconds"));
        // A fresh read MUST reflect the external change (no stale pre-mutation data).
        const rows = yield* Stream.runCollect(e.logStream(logQuery({ repoId, limit: 10 })));
        return { events, oids: rows.map((r: CommitSummary) => r.oid), external };
      }),
    );
