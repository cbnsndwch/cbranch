...
export interface GitEngineApi {
  ...
  readonly branchList: (repoId: RepoId) => Effect.Effect<BranchListing, GitError>;
  /** branch.create ✎ */
  ...
  /** branch.rename ✎ */
  readonly branchRename: (repoId: RepoId, oldName: string, newName: string) => Effect.Effect<void, GitError>;
  /** branch.delete ✎ — force=false = safe; force=true = -D after user confirm. */
  readonly branchDelete: (repoId: RepoId, name: string, force: boolean) => Effect.Effect<void, GitError>;
  /** branch.setUpstream ✎ — upstream=undefined = unset. */
  readonly branchSetUpstream: (repoId: RepoId, name: string, upstream?: string) => Effect.Effect<void, GitError>;

  // ── merge (P3) ─────────────────────────────────────────────────────────────
  /** merge.create ✎ */
  readonly mergeCreate: (repoId: RepoId, ref: string, strategy: MergeMode) => Effect.Effect<MergeResult, GitError>;
  /** merge.abort ✎ */
  ...
  // ── remotes (P3) ───────────────────────────────────────────────────────────
  /** remote.list */
  readonly remoteList: (repoId: RepoId) => Effect.Effect<ReadonlyArray<RemoteInfo>, GitError>;
  /** remote.add ✎ */
  readonly remoteAdd: (repoId: RepoId, name: string, url: string) => Effect.Effect<void, GitError>;
  /** remote.setUrl ✎ */
  readonly remoteSetUrl: (repoId: RepoId, name: string, url: string, push?: boolean) => Effect.Effect<void, GitError>;
  /** remote.rename ✎ */
  readonly remoteRename: (repoId: RepoId, oldName: string, newName: string) => Effect.Effect<void, GitError>;
  /** remote.remove ✎ */
  readonly remoteRemove: (repoId: RepoId, name: string) => Effect.Effect<void, GitError>;

  // ── worktrees (P3) ─────────────────────────────────────────────────────────
  /** worktree.list */
  readonly worktreeList: (repoId: RepoId) => Effect.Effect<ReadonlyArray<WorktreeInfo>, GitError>;
  /** worktree.add ✎ */
  ...
  /** worktree.remove ✎ */
  readonly worktreeRemove: (repoId: RepoId, path: string, force?: boolean) => Effect.Effect<void, GitError>;
  /** worktree.prune ✎ */
  ...
  /** stash.list */
  readonly stashList: (repoId: RepoId) => Effect.Effect<ReadonlyArray<StashEntry>, GitError>;
  /** stash.show */
  readonly stashShow: (repoId: RepoId, ref: string) => Effect.Effect<ReadonlyArray<DiffFile>, GitError>;
  /** stash.apply ✎ */
  readonly stashApply: (repoId: RepoId, ref: string) => Effect.Effect<void, GitError>;
  /** stash.pop ✎ */
  readonly stashPop: (repoId: RepoId, ref: string) => Effect.Effect<void, GitError>;
  /** stash.drop ✎ */
  readonly stashDrop: (repoId: RepoId, ref: string) => Effect.Effect<void, GitError>;
  /** stash.clear ✎ */
  readonly stashClear: (repoId: RepoId) => Effect.Effect<void, GitError>;

  // ── tags (P3) ──────────────────────────────────────────────────────────────
  /** tag.list */
  readonly tagList: (repoId: RepoId) => Effect.Effect<ReadonlyArray<TagInfo>, GitError>;
  /** tag.create ✎ */
  readonly tagCreate: (
    repoId: RepoId,
    name: string,
  ...
  /** tag.delete ✎ */
  readonly tagDelete: (repoId: RepoId, name: string) => Effect.Effect<void, GitError>;
  /** tag.push ✎ */
  readonly tagPush: (repoId: RepoId, remote: string, name?: string, all?: boolean) => Effect.Effect<void, GitError>;
  /** tag.deleteRemote ✎ */
  readonly tagDeleteRemote: (repoId: RepoId, remote: string, name: string) => Effect.Effect<void, GitError>;
