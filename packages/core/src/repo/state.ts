...
export const detectInProgress = (gitDir: string): InProgress => {
  const has = (...segments: string[]): boolean => existsSync(join(gitDir, ...segments));
...
export const readRepoState = (repo: ResolvedRepo): Effect.Effect<RepoState, GitError> =>
  Effect.gen(function* () {
    const cwd = repoCwd(repo);

    // Emptiness: a non-zero exit on `--verify HEAD` means an unborn branch (DATA).
    const headProbe = yield* runGit({ cwd, args: ["rev-parse", "--quiet", "--verify", "HEAD"] });
    const isEmpty = headProbe.exitCode !== 0;
    const verifiedHead = decodeUtf8(headProbe.stdout).trim();

    let headOid: Oid | undefined = verifiedHead === "" ? undefined : OidBrand.make(verifiedHead);
    let currentBranch: string | undefined;
    let isDetached = false;

    if (repo.isBare) {
      const sym = yield* runGit({ cwd, args: ["symbolic-ref", "--quiet", "--short", "HEAD"] });
      if (sym.exitCode === 0) currentBranch = decodeUtf8(sym.stdout).trim() || undefined;
      else isDetached = headOid !== undefined;
    } else {
      const status = yield* runGit({ cwd, args: ["status", "--porcelain=v2", "-z", "--branch"] });
      ...
      if (header.oid !== undefined && header.oid !== "(initial)") headOid = OidBrand.make(header.oid);
    }
...
export const parseBranchHeader = (stdout: Buffer): { oid?: string; head?: string } => {
  const records = decodeUtf8(stdout).split("\0");
  const out: { oid?: string; head?: string } = {};
  for (const record of records) {
    if (record.startsWith("# branch.oid ")) out.oid = record.slice("# branch.oid ".length).trim();
    else if (record.startsWith("# branch.head ")) out.head = record.slice("# branch.head ".length).trim();
  }
  return out;
};
